...
type AddressLike = { country: string; state: string; city: string };
...
async function resolveBestMethodCost(input: {
  ...
  const available = results.filter((r): r is NonNullable<typeof r> => Boolean(r));
 ...
 */
async function resolveBaseShipping(input: { subtotal: number; address: AddressLike }) {
...
export async function quoteShippingForUser(input: { userId: string; addressId: string }) {
  ...
  const quotes: {
    method: { id: string; type: string; name: string; description?: string | null };
    cost: number;
    ruleId: string;
    zoneId: string;
  }[] = [];
