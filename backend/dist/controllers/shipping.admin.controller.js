...
function normalizeZoneInput(input) {
    ...
    if (input.scope === "COUNTRY") {
        if (!country)
            throw new ApiError(400, "INVALID_ZONE", "Country is required for country scope");
        return { scope: input.scope, country, state: null, city: null, isActive: input.isActive ?? true };
    }
    if (input.scope === "STATE") {
        if (!country || !state)
            throw new ApiError(400, "INVALID_ZONE", "Country and state are required for state scope");
...
export const updateZone = asyncHandler(async (req, res) => {
    ...
    if (!existing)
        throw new ApiError(404, "ZONE_NOT_FOUND", "Shipping zone not found");
    const merged = {
...
export const createRule = asyncHandler(async (req, res) => {
    const { zoneId, methodId, baseCost, priority, isActive, conditionType, minOrderValue } = req.body;
    const finalConditionType = conditionType ?? "NONE";
    const conditionConfig = finalConditionType === "MIN_ORDER_VALUE"
