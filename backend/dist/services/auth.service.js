...
export async function registerUser(input) {
    ...
    const payload = { sub: user.id, firstName: user.firstName || user.name, role: user.role };
    return {
...
export async function startRegistration(input) {
    ...
    if (existing)
        throw new ApiError(409, "EMAIL_IN_USE", "Email is already in use");
    const existingIntent = await redis.get(intentKey(email));
    if (existingIntent) {
...
export async function verifyRegistrationOtp(input) {
    const email = input.email.toLowerCase().trim();
    const intentRaw = await redis.get(intentKey(email));
    if (!intentRaw)
        throw new ApiError(400, "REGISTRATION_EXPIRED", "Registration expired");
    const storedOtpHash = await redis.get(otpKey(email));
    if (!storedOtpHash)
...
export async function resendRegistrationOtp(input) {
    ...
    if (!intentRaw)
        throw new ApiError(400, "REGISTRATION_EXPIRED", "Registration expired");
    const existingOtp = await redis.get(otpKey(email));
    if (existingOtp)
        throw new ApiError(409, "OTP_STILL_VALID", "OTP still valid");
