...
export async function listProducts(input) {
    ...
    if (sortBy === "type")
        orderBy.push({ type: sortDir });
    if (sortBy === "category")
        orderBy.push({ category: { name: sortDir } });
    if (sortBy === "price")
        orderBy.push({ variants: { _min: { price: sortDir } } });
    if (sortBy === "stock")
        orderBy.push({ variants: { _sum: { stock: sortDir } } });
    if (sortBy === "createdAt")
...
export async function createProduct(input) {
    try {
        ...
        if (existingSkus.length > 0) {
            throw new ApiError(400, "SKU_EXISTS", `SKU(s) already exist: ${existingSkus.map((s) => s.sku).join(", ")}`);
        }
...
export async function updateProduct(id, input) {
    ...
    if (input.imageFileId !== undefined)
        data.imageFileId = input.imageFileId;
    if (input.imageFilePath !== undefined)
        data.imageFilePath = input.imageFilePath;
    if (input.thumbUrl !== undefined)
        data.thumbUrl = input.thumbUrl;
    if (input.thumbFileId !== undefined)
        data.thumbFileId = input.thumbFileId;
    if (input.thumbFilePath !== undefined)
        data.thumbFilePath = input.thumbFilePath;
