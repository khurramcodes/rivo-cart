...
async function mergeCarts(userCartId, sessionCartId) {
...
export async function resolveCart(userId, sessionId) {
    if (userId) {
...
async function getVariantForCart(productId, variantId) {
...
export async function addItemToCart(userId, sessionId, input) {
...
export async function migrateCartItems(userId, sessionId, items) {
    const context = await resolveCart(userId, sessionId);
    if (items.length === 0)
        return context;
...
export async function applyCouponToCart(userId, sessionId, code) {
    const context = await resolveCart(userId, sessionId);
...
export async function removeCouponFromCart(userId, sessionId) {
    const context = await resolveCart(userId, sessionId);
    await prisma.cart.update({
